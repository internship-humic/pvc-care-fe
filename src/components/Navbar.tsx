...
export default function Navbar({ userData }: { userData: any }) {
  ...
  // --- AUTO CLOSE MENU MOBILE PADA RESIZE KE DESKTOP ---
  useEffect(() => {
    const handleResize = () => {
  ...
  // --- FUNGSI STYLING MENU MOBILE ---
  const getMobileMenuClass = (path: string) => {
    return pathname === path
      ? "flex items-center gap-3 px-4 py-3 bg-[#4880FF] text-white rounded-xl text-sm font-semibold shadow-sm shadow-blue-500/20 transition"
      : "flex items-center gap-3 px-4 py-3 text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-xl text-sm font-medium transition";
  };

  return (
    <nav className="bg-white border-b border-slate-200/80 sticky top-0 z-40 shadow-sm">
      {/* BAR NAVIGASI UTAMA (TOP BAR) */}
      <div className="px-6 lg:px-10 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="md:hidden p-2 -ml-2 rounded-lg text-slate-600 hover:bg-slate-100 hover:text-slate-800 transition focus:outline-none focus:ring-2 focus:ring-slate-200"
        ...
        <div className="hidden md:flex items-center gap-2">
          {navItems.filter(item => item.show).map(item => (
            <Link key={item.path} href={item.path} className={getMenuClass(item.path)}>
              {item.icon(item.label === 'Kelola Dokter' ? 'w-5 h-5' : 'w-4 h-4')}
              {item.label}
            </Link>
          ))}
        </div>
      ...
      {/* DROPDOWN MENU MOBILE (Hanya muncul jika isMobileMenuOpen true dan ukuran layar < md) */}
      {isMobileMenuOpen && (
        <div className="md:hidden border-t border-slate-100 bg-white px-6 py-4 flex flex-col gap-2 animate-fade-in-up">
          {navItems.filter(item => item.show).map(item => (
            <Link 
              key={item.path} 
              href={item.path} 
              className={getMobileMenuClass(item.path)}
